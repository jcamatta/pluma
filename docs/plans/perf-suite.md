...
3. **Report assembly + renderer.** Split into two commits for reviewability:
   ...
   - **3b — markdown render.** `support/format-value.ts` (pure ms/bytes/count formatting, tested) + `support/render-report.ts` (pure: `RunRecord` → markdown, one section per scenario, summaries via `summarize`, tested); the teardown also writes `perf-results/report.md`. A run then produces both artifacts.
