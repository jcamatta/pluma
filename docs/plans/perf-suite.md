...
1. **Perf harness skeleton.** `playwright.perf.config.ts` (testMatch `**/scenarios/*.perf.ts`, workers 1, reuse global-setup; teardown wired in step 3); `e2e/perf/support/run-context.ts` (commit hash via `git rev-parse`, app version from `package.json`, machine id from `os.hostname()`, timestamp); `e2e/perf/support/stats.ts` (median/p50/p95 — pure, with `__tests__/stats.test.ts`); `e2e/perf/support/write-pending.ts` (a scenario writes its result to `perf-results/.pending/<scenario>.json`); npm script `test:perf`; `.gitignore` entry `perf-results/`; add the new config file to `tsconfig.e2e.json` include and `eslint/e2e.mjs` files. Lands green with the stats unit tests.
2. **Cold-start scenario.** `e2e/perf/scenarios/cold-start.perf.ts`: N full launch→close cycles measuring launch → first window visible → app interactive (locator from the existing launcher spec); writes its pending piece. First real measurement lands in `perf-results/.pending/`. Delivers the end-to-end proof that a scenario produces data.
3. **Report assembly + renderer.** `e2e/perf/support/assemble-run.ts` (pure: pending pieces + run context → one run record) and `e2e/perf/support/render-report.ts` (pure: run record → markdown, one section per scenario), both with `__tests__`; a `globalTeardown` wires them — collect pending → write `perf-results/runs/<timestamp>-<commit>.json` → clear pending → write `perf-results/report.md`. At this point a run produces both artifacts.
...
- **"App interactive" marker for cold start.** Pluma launches into the launcher (folder not yet open); the precise locator is chosen in step 2 from the existing `launcher.e2e.ts`. `OPEN (resolved during step 2)`.
