...
// so the run continues. Resolves a plain ack Result; never throws across IPC.
...
export const handleSubmitToolResult = (
  ...
  const program = submitToolResult(message).pipe(Effect.provideService(RuntimeAgent, runtimeAgent))
