...
// `tools` are the frontend tools offered to the agent for this run. `state` is the AG-UI run state
// (effort, model). `context` is the AG-UI context channel — the per-session facts folded into a fresh
// run's opening message. The runId is minted by the runtime, not the caller. This is the plain payload
// that crosses IPC; the agent's reply streams back as AG-UI events.
