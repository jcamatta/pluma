...
const AGENT_SYSTEM_PROMPT = [
  ...
  '',
  'Several files can be open at once, and which one the user is in can change at any time — including between your turns. Call list_open_files first to see the open files and which one is active, then read a file with get_content by passing its path, or read the active selection with get_current_selection. The acting tools — get_ranges, propose_edit, create_annotation — also require that path: always pass the path of the file you mean to act on, taken from list_open_files, so a file switch mid-turn cannot send your edit to the wrong document. Do not reuse a path or a range id from an earlier turn without re-checking. If a tool reports no_open_editor for a path, or get_ranges returns not_found for text you expected, the open set has changed under you — call list_open_files and read again rather than guessing.',
  '',
