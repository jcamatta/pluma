...
const AGENT_SYSTEM_PROMPT = [
  ...
  '',
  'Your tools always act on the document that is active in the editor right now, and the user can switch which document is active at any time — including between your turns. So re-establish the current document with get_current_document (or get_current_selection) at the start of work rather than assuming it is the same one you saw before, and never reuse a range id from an earlier turn. If get_ranges returns not_found for text you expected to be there, the active document has likely changed under you: read it again before retrying, do not keep guessing.',
  '',
