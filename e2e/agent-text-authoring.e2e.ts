...
// insert_at / insert_after tools to write MORE THAN ONE PARAGRAPH wherever it wants, and that the
// proposal applies as real paragraph nodes (not collapsed into one inline block) on Accept.
//
// Two scenarios:
...
//  B. Insert in the MIDDLE — a new paragraph after a named first sentence via insert_after. After Accept
//     the new paragraph lands BETWEEN the two original paragraphs, proving block-after-block placement.
//
...
const INSERT_AFTER_PROMPT =
  ...
  `"${FIRST_SENTENCE}" — use your insert_after tool with that sentence as the anchor. The new ` +
  'paragraph must read as the morning continuing, before the market fills. First call list_open_files ' +
  'to find the path of the active file, then pass that path and the anchor to insert_after in a single ' +
  'call. Do not reply with prose and do not ask for confirmation.'
...
test('inserts a new paragraph in the middle, after a named sentence, via insert_after', async () => {
  await withTempFolder([{ name: MIDDLE_FILE, content: MIDDLE_CONTENT }], async (folder) => {
    ...
    try {
      ...
      // Wire-arg proof: the model emitted insert_after with a non-empty path and markdown text.
      const calls = await readToolCalls((fn) => window.evaluate(fn))
      const insertAfter = calls.find((call) => call.toolName === 'insert_after')
      expect(
        insertAfter,
        `tool calls seen: ${calls.map((c) => c.toolName).join(', ')}`
      ).toBeDefined()
      const args = insertAfter?.args
      expect(hasStringField(args, 'path')).toBe(true)
      expect(hasStringField(args, 'text')).toBe(true)
    } finally {
      await app.close()
